	...
	function crearEsfera (pisos, r)
	{
		...
		for(var i = 0; i   <  pisos/2  ; i++)
		{
			...
			{	
				...
				for(var k = 0; k < 12 ; k++)
				{
					normales.push(2*vertices[i*pisos*12 + 12*j + k]);
				}
	...
	function crearCilindroArticuladoConEje (cantidadCuadros, r, eje)
	{
		...
		for(var j =0; j < puntos - 1; j++)
		{
			...
			{	
				...
				normales2 = [];
				normales2.push(2*vertices2[0]);
				normales2.push(0);
				normales2.push(2*vertices2[2]);
				
				normales2.push(2*vertices2[3]);
				normales2.push(0);
				normales2.push(2*vertices2[5]);
				...
				temp1 = rotarTransladar([eje[3*(j+1)],eje[3*(j+1)+1], eje[3*(j+1)+2]],vector, normales2, temp1.cambioBase);
				normales.push.apply(normales,temp1.vertices);
		...
		var ret = {vertices : vertices, coords : coords, textCords : textCords, puntosEje : puntosEje, normales : normales};
		return ret;
	}
